...
function Admin() {
  ...
  const [paymentMethods, setPaymentMethods] = useState(() => {
    const saved = localStorage.getItem('bharatfunded-payment-methods');
  ...
  const [fundRequests, setFundRequests] = useState(() => {
    const saved = localStorage.getItem('bharatfunded-fund-requests');
    return saved ? JSON.parse(saved) : [];
  });
  ...
  const [currencySettings, setCurrencySettings] = useState(() => {
    const saved = localStorage.getItem('bharatfunded-currency-settings');
    return saved ? JSON.parse(saved) : {
      usdMarkup: 0,
      lastUpdated: null
    };
  ...
  const [tradeModeSettings, setTradeModeSettings] = useState(() => {
    const saved = localStorage.getItem('bharatfunded-trade-mode-settings');
    return saved ? JSON.parse(saved) : {
      hedging: {
        enabled: true,
        minLotSize: 0.01,
  ...
  useEffect(() => {
    const verifyAdmin = async () => {
      ...
      const savedUser = localStorage.getItem('bharatfunded-admin-user');
      if (!token || !savedUser) {
  ...
  const handleFundRequest = (requestId, action) => {
    const updatedRequests = fundRequests.map(req => {
      if (req.id === requestId) {
        ...
        if (action === 'approve') {
          ...
          localStorage.setItem('bharatfunded-users', JSON.stringify(users));

          // Also update bharatfunded-auth if this is the currently logged in user
    ...
    localStorage.setItem('bharatfunded-fund-requests', JSON.stringify(updatedRequests));
  };
  ...
  const saveCurrencySettings = async () => {
    ...
    localStorage.setItem('bharatfunded-currency-settings', JSON.stringify(settings));
    try {
      const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:3001';
  ...
  const saveTradeModeSettings = (mode) => {
    localStorage.setItem('bharatfunded-trade-mode-settings', JSON.stringify(tradeModeSettings));
    ...
    <div className="admin-container">
      ...
      <aside className={`admin-sidebar ${sidebarCollapsed ? 'collapsed' : ''} ${mobileMenuOpen ? 'mobile-open' : ''}`}>
        <div className="sidebar-header">
          <span className="sidebar-logo">{sidebarCollapsed ? 'SP' : 'BharatFundedTrade Admin'}</span>
          <button
