...
const RiskManagement = () => {
  ...
  const fetchGlobalSettings = useCallback(async () => {
    try {
      ...
      const res = await fetch(`${API_URL}/api/admin/risk-settings`, {
        headers: { 'Authorization': `Bearer ${localStorage.getItem('bharatfunded-admin-token')}` }
      });
  ...
  const saveGlobalSettings = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/admin/risk-settings`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${localStorage.getItem('bharatfunded-admin-token')}` },
  ...
  const resetUserToDefaults = async () => {
    ...
    try {
      await fetch(`${API_URL}/api/admin/user-risk-settings/${selectedUser._id}`, {
        method: 'DELETE', headers: { 'Authorization': `Bearer ${localStorage.getItem('bharatfunded-admin-token')}` }
      });
