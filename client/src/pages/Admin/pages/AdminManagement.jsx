...
function AdminManagement() {
  ...
  const fetchFundRequests = async () => {
    setLoading(true);
    try {
      // Get current admin from localStorage - check both keys (super admin uses bharatfunded-admin-user)
      let adminData = JSON.parse(localStorage.getItem('bharatfunded-admin') || '{}');
      if (!adminData._id) {
        // Try bharatfunded-admin-user for super admin
        adminData = JSON.parse(localStorage.getItem('bharatfunded-admin-user') || '{}');
      }
  ...
  return (
    <div className="admin-page-container">
      <div className="admin-page-header">
        <h2>{getTabTitle()}</h2>
        <button onClick={() => {
          const nextRole = activeTab === 'brokers' ? 'broker' : 'sub_admin';
          setForm({
            name: '', email: '', phone: '', password: '', newPassword: '',
            role: nextRole,
            parentId: '',
            isActive: true,
            permissions: ROLE_PRESETS[nextRole] || ROLE_PRESETS.broker,
          });
