...
function MarketPage() {
  ...
  // Active Trades drilldown (Phase 1) — netting only.
  ...
  const fetchLegsForPosition = useCallback(
    async (positionOderId) => {
      ...
      try {
        ...
        const data = await res.json();
        // Only open legs are "active trades" — closed/consumed/partial_close legs
        // belong in History, not the Active Trades panel.
    ...
    const fetchSettings = async () => {
      ...
      try {
        ...
        const res = await fetch(
          ...
          { headers: { Authorization: `Bearer ${localStorage.getItem('bharatfunded-token')}` } }
        );
        const data = await res.json();
        if (data.success) {
          ...
          <>
            ...
            <div className="order-input-group sl-tp">
              ...
              {showStopLoss && (
                <>
                  ...
                  {(() => {
                    // SL placement hint — Fix 14. Mirrors the limit-price band
                    // hint and reflects the same server-side validation in
                    // NettingEngine.validateSLTPPlacement (Fix 12).
                    // BUY: SL must be <= market - away. SELL: SL >= market + away.
                    ...
                    if (!band) {
                      return (
                        <span style={{ fontSize: '10px', color: '#71717a', marginTop: '4px', display: 'block' }}>
                          {orderSide === 'buy'
                            ? `Must be below ${formatPrice(ref, selectedSymbol)}`
                            : `Must be above ${formatPrice(ref, selectedSymbol)}`}
                        </span>
                      );
                    }
                    const limit = orderSide === 'buy' ? ref - band.away : ref + band.away;
                    return (
                      <span style={{ fontSize: '10px', color: '#71717a', marginTop: '4px', display: 'block' }}>
                        {orderSide === 'buy'
                          ? `At or below ${formatPrice(limit, selectedSymbol)} (${band.detail})`
                          : `At or above ${formatPrice(limit, selectedSymbol)} (${band.detail})`}
                      </span>
                    );
                  })()}
                </>
            ...
            <div className="order-input-group sl-tp">
              ...
              {showTakeProfit && (
                <>
                  ...
                  {(() => {
                    // TP placement hint — same band logic as SL but inverted.
                    // BUY: TP must be >= market + away. SELL: TP <= market - away.
                    const ref = entryPrice || 0;
                    if (!(ref > 0)) return null;
                    const band = getNettingStopSlmAwayOffset(ref, segmentSettings);
                    if (!band) {
                ...
                <div className="order-input-group sl-tp">
                  ...
                  {showPositionSize && (<>
                    ...
                    <div className="margin-info" style={{ marginTop: '8px', fontSize: '12px', color: 'var(--text-secondary)' }}>
                      {isTimesM && timesX > 0
                        ? `Effective leverage: ${effectiveMultiplier}X`
                        : `Effective: 1:${Math.round(10000 / leverage) / 100}x leverage`}
                    </div>
                  </>)}
                </div>
              );
            ...
            <div className="trading-charges">
              {tradingMode === 'netting' && segmentSettings && (() => {
                ...
                const isIndian = isIndianInstrument(selectedSymbol);
                const currSym = '₹'; // Admin always sets in INR

                // Fix 24: detect expiry day for display
                const isExpiryToday = (() => {
                  if (!selectedInstrument?.expiry) return false;
                  const exp = new Date(selectedInstrument.expiry);
                  if (isNaN(exp.getTime())) return false;
                ...
                // Check if expiry-day margin is actually set for this side
                const isOptSeg = resolvedSegmentApiName && ['NSE_OPT', 'BSE_OPT', 'MCX_OPT', 'CRYPTO_OPTIONS'].includes(resolvedSegmentApiName);
                const hasExpiryMargin = isExpiryToday && (
                  isOptSeg
                    ? Number(orderSide === 'buy' ? segmentSettings.expiryDayOptionBuyMargin : segmentSettings.expiryDayOptionSellMargin) > 0
                    : Number(segmentSettings.expiryDayIntradayMargin) > 0
                );

                let modeLabel, modeColor;
                if (hasExpiryMargin) {
                  modeLabel = `Expiry Day — ${currSym}${X}`;
                  modeColor = '#f59e0b';
                } else if (mode === 'times') {
                ...
                return (
                  <>
                    ...
                    {hasExpiryMargin && (
                      <div className="charge-row" style={{ fontSize: '11px', color: '#f59e0b' }}>
                        <span>Expiry Day</span>
                        <span style={{ fontWeight: '600' }}>Active — {orderSide === 'buy' ? 'Buy' : 'Sell'} side margin applied</span>
                      </div>
                    )}
                  </>
                );
              })()}
              {/* On expiry day with expiry margin set → show only expiry margin row */}
              {(() => {
                const isExpiryToday = (() => {
                  if (!selectedInstrument?.expiry) return false;
                  const exp = new Date(selectedInstrument.expiry);
                ...
                const isOptSeg = resolvedSegmentApiName && ['NSE_OPT', 'BSE_OPT', 'MCX_OPT', 'CRYPTO_OPTIONS'].includes(resolvedSegmentApiName);
                const hasExpiryMargin = isExpiryToday && segmentSettings && (
                  isOptSeg
                    ? Number(orderSide === 'buy' ? segmentSettings.expiryDayOptionBuyMargin : segmentSettings.expiryDayOptionSellMargin) > 0
                    : Number(segmentSettings.expiryDayIntradayMargin) > 0
                );
                ...
                if (hasExpiryMargin) {
                  return (
                    <div className="charge-row">
                      <span>Expiry Day Margin</span>
                      <span style={{ color: '#f59e0b', fontWeight: '600' }}>{formatMargin(calculateRequiredMargin('intraday'))}</span>
                    </div>
                  );
                }
                return (
                  <>
                    <div className="charge-row"><span>Intraday Margin</span><span style={{ color: '#3b82f6', fontWeight: '600' }}>{formatMargin(calculateRequiredMargin('intraday'))}</span></div>
                    <div className="charge-row">
                      <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                        Carryforward Margin
                        {overnightDisabled && (
                          <span title="Overnight/carryforward is disabled for this segment" style={{ cursor: 'help', fontSize: '10px', backgroundColor: '#333', color: '#a1a1aa', padding: '2px 4px', borderRadius: '4px' }}>OFF</span>
                        )}
                      </span>
                      <span style={{ color: overnightDisabled ? '#a1a1aa' : '#3b82f6', fontWeight: '600' }}>
                        {formatMargin(calculateRequiredMargin('carryforward'))}
                      </span>
                    </div>
                  </>
                );
              ...
              {tradingMode === 'netting' && segmentSettings?.marginCalcMode === 'times' && (() => {
                ...
                return (
                  <div className="charge-row"><span>Effective Leverage ({leverage}%)</span><span style={{ color: '#22c55e', fontWeight: '600' }}>{effectiveX}X</span></div>
                );
              })()}
  ...
  return (
    <div className="market-page-root">
      ...
      <div className={`market-section chart-section ${mobileMarketTab === 'chart' ? 'active' : ''}`}>
        ...
        {/* Order Book — resizable via mousedown-hold-drag handle (Fix 15b).
            While the mouse button is held, a transparent full-viewport
            overlay sits ABOVE the TradingView iframe (z-index 99999) so
            the cursor moving over the chart doesn't break tracking. The
            overlay's React onMouseMove/onMouseUp handlers replace any
            window-level listeners, eliminating the iframe-capture issue. */}
        {isResizingOrderBook && (
          <div
            onMouseMove={(e) => {
              if (orderBookResizeRafRef.current != null) return;
              const clientY = e.clientY;
              orderBookResizeRafRef.current = requestAnimationFrame(() => {
                orderBookResizeRafRef.current = null;
                setOrderBookHeight(computeOrderBookHeightFromY(clientY));
              });
            }}
            onMouseUp={() => setIsResizingOrderBook(false)}
            onMouseLeave={() => setIsResizingOrderBook(false)}
            style={{
              position: 'fixed',
              inset: 0,
              zIndex: 99999,
              cursor: 'ns-resize',
              // Fully transparent — pointerEvents auto (default) means it
              // captures every mouse event in the viewport, including over
              // the TradingView iframe underneath.
        ...
        <div
          className="order-book-resize-handle"
          onMouseDown={(e) => {
            // Press-and-hold pattern (Fix 15b): mousedown enters resize
            // mode, then the full-viewport overlay (rendered above) takes
            // over and tracks mousemove/mouseup directly. preventDefault
            // stops the browser from starting a text selection drag.
            e.preventDefault();
            setIsResizingOrderBook(true);
          }}
          title={isResizingOrderBook ? 'Click anywhere to drop' : 'Click to resize'}
          style={{
            height: 8,
            cursor: 'ns-resize',
            background: isResizingOrderBook
              ? 'linear-gradient(180deg, transparent 0%, rgba(96, 165, 250, 0.55) 50%, transparent 100%)'
              : 'linear-gradient(180deg, transparent 0%, rgba(255, 255, 255, 0.08) 50%, transparent 100%)',
            borderTop: '1px solid var(--border-color)',
            borderBottom: '1px solid var(--border-color)',
            position: 'relative',
            flexShrink: 0,
            transition: isResizingOrderBook ? 'none' : 'background 0.15s ease-out',
            userSelect: 'none'
          }}
        >
          <div
            style={{
              ...
              height: 3,
              borderRadius: 2,
              background: isResizingOrderBook ? '#60a5fa' : 'rgba(255, 255, 255, 0.25)',
              pointerEvents: 'none'
            }}
          />
        </div>
        <div
          ...
          style={{
            ...
            maxHeight: orderBookHeight,
            // Skip the height transition during active drag — any interp
            // would lag behind the mouse. Outside drag, a tiny ease keeps
            // programmatic height changes from snapping.
            transition: isResizingOrderBook ? 'none' : 'height 0.12s ease-out'
          }}
        >
                    ...
                    scopedPositions.map((pos) => {
                      ...
                      if (isBinary && pos.expiry && pos.createdAt) {
                        ...
                        binaryPnL = currentPrice > pos.entryPrice
                          ? (pos.direction === 'up' ? `+₹${(amount * 0.85).toFixed(2)}` : `-₹${amount.toFixed(2)}`)
                          : (pos.direction === 'down' ? `+₹${(amount * 0.85).toFixed(2)}` : `-₹${amount.toFixed(2)}`);
                      }
                      
                      const positionRowKey = pos._id || pos.tradeId;
                      const positionId = pos.oderId || pos._id;
                      const isNetting = pos.mode === 'netting';
                      const isExpanded = isNetting && expandedPositionId === positionId;
                      const expandedLegs = isExpanded ? (legsByPosition[positionId] || []) : null;
                      const isLoadingLegs = isExpanded && legsLoadingFor === positionId && !legsByPosition[positionId];
                      const positionContractSize = getContractSize(pos.symbol);
                      const sideMul = (pos.side || pos.direction) === 'sell' ? -1 : 1;

                      return (
                        <Fragment key={positionRowKey}>
                        <tr className={isBinary ? 'binary-row' : ''}>
                          <td>
                            {isBinary ? (
                              ...
                              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                                {isNetting && (
                                  <button
                                    type="button"
                                    onClick={() => toggleExpandPosition(pos)}
                                    title={isExpanded ? 'Hide active trades' : 'Show active trades'}
                                    style={{
                                      background: isExpanded ? 'rgba(59, 130, 246, 0.18)' : 'rgba(255, 255, 255, 0.08)',
                          ...
                          <td style={{ fontSize: '11px' }}>{pos.stopLoss ? formatPrice(pos.stopLoss, pos.symbol) : '—'}</td>
                          <td style={{ fontSize: '11px' }}>{pos.takeProfit ? formatPrice(pos.takeProfit, pos.symbol) : '—'}</td>
                        ...
                        {isExpanded && (
                          <tr className="active-trades-row">
                            <td colSpan="13" style={{ background: 'rgba(59, 130, 246, 0.05)', padding: '8px 12px 12px 32px', borderBottom: '1px solid #1f2937' }}>
                              <div style={{ fontSize: 11, color: '#9ca3af', marginBottom: 6, fontWeight: 600, letterSpacing: 0.3 }}>
                                ACTIVE TRADES ({expandedLegs?.length || 0})
                                <span style={{ marginLeft: 8, fontWeight: 400, color: '#6b7280' }}>
                                  individual fills that built this {(pos.side || '').toUpperCase()} {parseFloat(pos.volume).toFixed(2)} position
                                </span>
                              </div>
                              {isLoadingLegs ? (
                                <div style={{ color: '#9ca3af', fontSize: 12 }}>Loading…</div>
                              ) : !expandedLegs || expandedLegs.length === 0 ? (
                                <div style={{ color: '#9ca3af', fontSize: 12 }}>No open active trades for this position.</div>
                              ) : (
                                <table className="positions-table" style={{ marginTop: 0 }}>
                                  <thead>
                                    <tr style={{ fontSize: 10, color: '#6b7280' }}>
                                      <th>Time</th>
                                      <th>Side</th>
                                      <th>Vol</th>
                                      <th>Entry</th>
                                  ...
                                  <tbody>
                                    {expandedLegs.map((leg) => {
                                      ...
                                      const legPnl = (currentPrice - legEntry) * sideMul * legVol * positionContractSize;
                                      return (
                                        <tr key={leg._id || leg.tradeId}>
                                          <td>{new Date(leg.executedAt || leg.createdAt).toLocaleTimeString()}</td>
                                          <td className={leg.side}>{(leg.side || '').toUpperCase()}</td>
                                          <td>{legVol.toFixed(2)}</td>
                                          <td>{formatPrice(legEntry, pos.symbol)}</td>
                                          <td>{formatPrice(currentPrice, pos.symbol)}</td>
                                          <td style={{ fontSize: 11 }}>{leg.stopLoss ? formatPrice(leg.stopLoss, pos.symbol) : '—'}</td>
                                          <td style={{ fontSize: 11 }}>{leg.takeProfit ? formatPrice(leg.takeProfit, pos.symbol) : '—'}</td>
                                          <td className={legPnl >= 0 ? 'profit' : 'loss'}>
                                            {formatPnL(legPnl, pos.symbol)}
                                          </td>
                                          <td>
                                            <button
                                              className="act-btn"
                                              onClick={() => openLegEditModal(leg, pos)}
                                              title="Edit per-fill SL/TP"
                                            >
                                              ✎
                                            </button>
                                            <button
                                              className="act-btn close"
                                              onClick={() => closeLegHandler(leg, pos)}
                  ...
                  {scopedPendingOrders.length === 0 ? (
                    ...
                    scopedPendingOrders.map((order) => (
                      <tr key={order._id || order.tradeId}>
                        ...
                        <td>{order.volume}</td>
                        <td>{formatPrice(order.price || order.entryPrice, order.symbol)}</td>
                        <td style={{ fontSize: '11px' }}>{order.stopLoss ? formatPrice(order.stopLoss, order.symbol) : '—'}</td>
                        <td style={{ fontSize: '11px' }}>{order.takeProfit ? formatPrice(order.takeProfit, order.symbol) : '—'}</td>
                        <td>
                    ...
                    <tbody>
                      {scopedTradeHistory.length === 0 ? (
                        ...
                        paginatedOrderBookHistory.map((trade) => {
                          ...
                          // Fix 18: a row is expandable iff it has a groupId.
                          // Children are rendered inline below the parent when
                          // expandedHistoryGroupId === trade.groupId.
                          const groupId = trade.groupId || null;
                          const isExpandable = !!groupId;
                          const isExpanded = isExpandable && expandedHistoryGroupId === groupId;
                          const isLoadingChildren = isExpanded && historyChildrenLoadingFor === groupId && !historyChildrenByGroup[groupId];
                          const children = isExpanded ? (historyChildrenByGroup[groupId] || []) : null;
                          const remarkColor = trade.remark === 'SL' || trade.remark === 'SL (per-fill)' ? '#ef4444'
                            : trade.remark === 'TP' || trade.remark === 'TP (per-fill)' ? '#10b981'
                            : trade.remark === 'Stop Out' ? '#dc2626'
                            : trade.remark === 'Aggregate Close' ? '#9ca3af'
                            : trade.remark === 'Auto Square-Off' ? '#f59e0b'
                            : '#9ca3af';
                          return (
                            <Fragment key={trade._id || trade.tradeId}>
                              <tr>
                                <td>
                                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
                                    {isExpandable && (
                                      <button
                                        type="button"
                                        onClick={() => toggleExpandHistoryRow(groupId)}
                                        title={isExpanded ? 'Hide details' : 'Show close legs'}
                                        style={{
                                          background: isExpanded ? 'rgba(59, 130, 246, 0.18)' : 'rgba(255, 255, 255, 0.06)',
                                          border: '1px solid rgba(59, 130, 246, 0.45)',
                                          color: '#60a5fa',
                                          cursor: 'pointer',
                                          padding: 0,
                                    ...
                                    {new Date(trade.closeTime || trade.closedAt || trade.createdAt).toLocaleTimeString()}
                                  </span>
                                </td>
                                <td>{trade.symbol}</td>
                                ...
                                <td style={{ fontSize: '11px', color: remarkColor }}>{trade.remark || trade.closedBy || '—'}</td>
                              </tr>
                              {isExpanded && (
                                <tr className="history-children-row">
                                  <td colSpan="11" style={{ background: 'rgba(59, 130, 246, 0.05)', padding: '8px 12px 12px 32px', borderBottom: '1px solid #1f2937' }}>
                                    <div style={{ fontSize: 11, color: '#9ca3af', marginBottom: 6, fontWeight: 600, letterSpacing: 0.3 }}>
                                      CLOSE DETAILS ({children?.length || 0} leg{(children?.length || 0) === 1 ? '' : 's'})
                                      <span style={{ marginLeft: 8, fontWeight: 400, color: '#6b7280' }}>
                                        — every fill that was closed by this {trade.remark || 'action'}
                                      </span>
                                    </div>
                                    {isLoadingChildren ? (
                                      <div style={{ color: '#9ca3af', fontSize: 12 }}>Loading…</div>
                                    ) : !children || children.length === 0 ? (
                                      <div style={{ color: '#9ca3af', fontSize: 12 }}>No leg-level audit rows found for this group.</div>
                                    ) : (
                                      <table className="positions-table" style={{ marginTop: 0 }}>
                                        <thead>
                                          <tr style={{ fontSize: 10, color: '#6b7280' }}>
                                        ...
                                        <tbody>
                                          {children.map((child) => {
                                            const childPnl = Number(child.profit) || 0;
                                            const childRemarkColor = child.remark === 'SL' || child.remark === 'SL (per-fill)' ? '#ef4444'
                                              : child.remark === 'TP' || child.remark === 'TP (per-fill)' ? '#10b981'
                                              : child.remark === 'Stop Out' ? '#dc2626'
                                              : child.remark === 'Aggregate Close' ? '#9ca3af'
                                              : '#9ca3af';
                                            const typeLabel = child.type === 'consumed' ? 'Consumed'
                                              : child.type === 'partial_close' ? 'Partial'
                                              : child.type === 'close' ? 'Close'
                                              : child.type;
                                            const typeColor = child.type === 'consumed' ? '#9ca3af'
                                              : child.type === 'partial_close' ? '#f59e0b'
                                              : '#ef4444';
                                            return (
                                              <tr key={child._id || child.tradeId}>
                                                <td>{new Date(child.closedAt || child.executedAt || child.createdAt).toLocaleTimeString()}</td>
                                                <td style={{ fontSize: 11, color: typeColor }}>{typeLabel}</td>
                                                <td>{Number(child.volume || 0).toFixed(2)}</td>
                                                <td>{formatPrice(child.entryPrice, child.symbol)}</td>
                                                <td>{formatPrice(child.closePrice, child.symbol)}</td>
                                                <td className={childPnl >= 0 ? 'profit' : 'loss'}>
                                                  {childPnl !== 0 ? formatPnL(childPnl, child.symbol) : '—'}
                                                </td>
                                                <td style={{ fontSize: 11, color: childRemarkColor }}>{child.remark || child.closedBy || '—'}</td>
                                              </tr>
                                            );
                                          })}
                                        </tbody>
