...
export const useUserPreferences = (user) => {
  ...
  // Get auth token
  const getToken = () => {
    return localStorage.getItem('bharatfunded-token');
  };
  ...
  const loadFromLocalStorage = () => {
    try {
      const displayCurrency = localStorage.getItem('bharatfunded-display-currency') || 'INR';
      const darkMode = localStorage.getItem('bharatfunded-dark-mode');
      const activePage = localStorage.getItem('bharatfunded-active-page') || 'home';
      const watchlistStr = localStorage.getItem('bharatfunded-watchlist');
      setPreferences(prev => {
        ...
        if (Array.isArray(wl)) {
          ...
          if (JSON.stringify(merged) !== JSON.stringify(wl)) {
            wl = merged;
            localStorage.setItem('bharatfunded-watchlist', JSON.stringify(wl));
          }
        }
        return {
          ...prev,
  ...
  const migrateLocalStorageToDb = async (dbPrefs) => {
    ...
    if (!hasDbWatchlist) {
      const localWatchlist = localStorage.getItem('bharatfunded-watchlist');
      if (localWatchlist) {
        try {
    ...
    const localCurrency = localStorage.getItem('bharatfunded-display-currency');
    if (localCurrency && !dbPrefs.displayCurrency) {
      await updatePreference('displayCurrency', localCurrency);
    }
    
    // Migrate dark mode
    const localDarkMode = localStorage.getItem('bharatfunded-dark-mode');
  ...
  const saveToDatabase = useCallback(async (updates) => {
    if (!isLoggedIn()) {
      ...
      Object.entries(updates).forEach(([key, value]) => {
        if (key === 'watchlist') {
          localStorage.setItem('bharatfunded-watchlist', JSON.stringify(value));
        } else if (key === 'displayCurrency') {
          localStorage.setItem('bharatfunded-display-currency', value);
        } else if (key === 'darkMode') {
          localStorage.setItem('bharatfunded-dark-mode', String(value));
        } else if (key === 'activePage') {
          localStorage.setItem('bharatfunded-active-page', value);
        }
      });
      return;
    }
