...
import { SubAdminDashboard, SubAdminUsers, SubAdminBrokers, SubAdminWallet, SubAdminTrades, SubAdminFunds, SubAdminBrokerFunds, SubAdminBankManagement, SubAdminSettings } from './pages/SubAdmin/pages';
import { ScopedSegmentUserSettings, ScopedSegmentMatrix, ScopedScriptMatrix, ScopedCopySettings, ScopedRiskOverride, ScopedAuditLog } from './pages/shared/scoped';
import { ScopedUserList, ScopedKycList, ScopedUserActivityLogs } from './pages/shared/userManagement';
import { ScopedTradeList } from './pages/shared/tradeManagement';
import { ScopedTransactionsList } from './pages/shared/fundManagement';
import BrokerLayout from './pages/Broker/BrokerLayout';
import { BrokerDashboard, BrokerUsers, BrokerWallet, BrokerTrades, BrokerFunds, BrokerBankManagement, BrokerSettings } from './pages/Broker/pages';
...
function WalletPage({ user }) {
  ...
  const fetchWalletFromServer = async () => {
    ...
    const userId = user?.id || user?.oderId || JSON.parse(localStorage.getItem('bharatfunded-auth') || '{}')?.user?.id;
    if (!userId) {
  ...
  useEffect(() => {
    ...
    const loadPaymentMethods = async () => {
      try {
        ...
        const raw = JSON.parse(localStorage.getItem('bharatfunded-payment-methods') || '{}');
        setPaymentMethods({
...
function QuickWalletActions() {
  ...
  const fetchWallet = () => {
    try {
      ...
      // Get from bharatfunded-auth (logged in user data)
      ...
      if (authData.user) {
        ...
        // Also check bharatfunded-users for admin-updated wallet
        const users = JSON.parse(localStorage.getItem('bharatfunded-users') || '[]');
...
function App({ user, onLogout }) {
  ...
  useEffect(() => {
    localStorage.setItem('bharatfunded-active-page', activePage);
  }, [activePage]);
    ...
    <BrowserRouter>
      ...
      <Routes>
        ...
        <Route path="/subadmin-panel" element={<SubAdminLayout />}>
          ...
          <Route path="users/legacy" element={<SubAdminUsers />} />
          ...
          <Route path="trades/legacy" element={<SubAdminTrades />} />
          ...
          <Route path="funds/legacy" element={<SubAdminFunds />} />
