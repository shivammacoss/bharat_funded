# Stock4x Deployment Guide - AWS EC2 + MongoDB Atlas + Cloudflare

## Prerequisites
- Fresh AWS EC2 instance (Ubuntu 22.04 LTS recommended)
- Domain: stock4x.com connected to Cloudflare
...
2. Create a new project: `Stock4x`
...
```
mongodb+srv://stock4x_admin:<password>@cluster0.xxxxx.mongodb.net/stock4x?retryWrites=true&w=majority
```
...
sudo mkdir -p /var/www/stock4x
sudo chown -R stock4x:stock4x /var/www/stock4x
cd /var/www/stock4x
```
...
**Option A: Using Git**
```bash
git clone https://github.com/YOUR_USERNAME/stockpip.git .
```

**Option B: Using SCP (from your local machine)**
```bash
# Run this on your LOCAL machine
scp -i your-key.pem -r /Users/tarundewangan/Downloads/Projects/stockpip/* ubuntu@YOUR_ELASTIC_IP:/var/www/stock4x/
```

### 4.3 Install Server Dependencies
```bash
cd /var/www/stock4x/server
npm install
```

### 4.4 Create Server Environment File
```bash
nano /var/www/stock4x/server/.env
```
...
MONGODB_URI=mongodb+srv://stock4x_admin:YOUR_PASSWORD@cluster0.xxxxx.mongodb.net/stock4x?retryWrites=true&w=majority

# JWT Secret (generate a strong random string)
JWT_SECRET=Stock4x-super-secret-key-change-this-in-production-2024
...
FRONTEND_URL=https://stock4x.com

# Admin Credentials
ADMIN_EMAIL=admin@stock4x.com
...
### 4.5 Build Client
```bash
cd /var/www/stock4x/client
npm install
```

Create client environment file:
```bash
nano /var/www/stock4x/client/.env.production
```

Add:
```env
VITE_API_BASE_URL=https://api.stock4x.com
VITE_WS_URL=wss://api.stock4x.com
```
...
### 5.1 Create Nginx Config for API (api.stock4x.com)
```bash
sudo nano /etc/nginx/sites-available/stock4x-api
```
...
### 5.2 Create Nginx Config for Frontend (stock4x.com)
```bash
sudo nano /etc/nginx/sites-available/stock4x-frontend
```
...
server {
    listen 80;
    server_name stock4x.com www.stock4x.com;

    root /var/www/stock4x/client/dist;
    index index.html;
...
### 5.3 Enable Sites
```bash
sudo ln -s /etc/nginx/sites-available/stock4x-api /etc/nginx/sites-enabled/
sudo ln -s /etc/nginx/sites-available/stock4x-frontend /etc/nginx/sites-enabled/
...
### 6.1 Add DNS Records
Go to Cloudflare Dashboard → stock4x.com → DNS
...
pm2 logs stock4x-api    # View logs
pm2 restart stock4x-api # Restart server
pm2 stop stock4x-api    # Stop server
...
### View Server Logs
```bash
pm2 logs stock4x-api --lines 100
```
...
# or from browser: https://api.stock4x.com/api/health
```

### 9.2 Test Frontend
Open https://stock4x.com in browser
...
| Service | URL |
|---------|-----|
| Frontend | https://stock4x.com |
| API | https://api.stock4x.com |
| WebSocket | wss://api.stock4x.com |
